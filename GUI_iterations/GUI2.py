...
class BwtApp:
    ...
    def create_widgets(self):
        # Labels
        ttk.Label(self.master, text="Choose an option:").grid(row=0, column=0, columnspan=2, pady=10)

        # Buttons
        ttk.Button(self.master, text="Perform BWT", command=self.perform_bwt).grid(row=1, column=0, pady=5)
        ttk.Button(self.master, text="Inverse BWT", command=self.inverse_bwt).grid(row=1, column=1, pady=5)
        ttk.Button(self.master, text="Calculate Size", command=self.calculate_size).grid(row=2, column=0, pady=5)
        ttk.Button(self.master, text="Measure Compression", command=self.measure_compression).grid(row=2, column=1, pady=5)
        ttk.Button(self.master, text="Exit", command=self.confirm_exit).grid(row=3, column=0, columnspan=2, pady=10)
    ...
    def confirm_exit(self):
        confirm = simpledialog.askstring("Exit", "Are you sure you want to exit?")
